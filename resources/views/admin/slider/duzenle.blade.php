...
<div class="admin-table-card" style="max-width:700px;">
    <div class="admin-table-header">
        <h5><i class="fas fa-edit" style="color:var(--admin-primary);margin-right:8px;"></i> Slider Düzenle</h5>
    ...
    <div style="padding:28px;">
        ...
        <form action="{{ route('admin.slider.duzenlePost', $slider->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group mb-3">
                ...
                <input type="text" class="form-control" name="adi" value="{{ old('adi', $slider->adi) }}" required style="height:46px;border-radius:8px;border:2px solid var(--admin-border);font-family:'Poppins',sans-serif;">
            </div>

            <div class="form-group mb-3">
                <label style="font-size:13px;font-weight:600;color:var(--admin-dark);margin-bottom:6px;display:block;">Link Adresi</label>
                <input type="text" class="form-control" name="link" value="{{ old('link', $slider->link) }}" placeholder="https://" style="height:46px;border-radius:8px;border:2px solid var(--admin-border);font-family:'Poppins',sans-serif;">
            </div>
            ...
            <div class="form-group mb-3">
                ...
                <select class="form-control" id="media_type" name="media_type" required style="height:46px;border-radius:8px;border:2px solid var(--admin-border);font-family:'Poppins',sans-serif;">
                    <option value="image" {{ old('media_type', $slider->media_type ?? 'image') === 'image' ? 'selected' : '' }}>Görsel</option>
                    <option value="video" {{ old('media_type', $slider->media_type ?? 'image') === 'video' ? 'selected' : '' }}>Video</option>
                </select>
            </div>
            ...
            <div class="form-group mb-3" id="imageField">
                ...
                @if($slider->resim)
                    <div class="mb-2">
                        <img src="{{ asset('tema/uploads/slider/' . $slider->resim) }}" alt="{{ $slider->adi }}" style="max-width:300px;border-radius:8px;">
                    </div>
                @endif
            ...
            <div class="form-group mb-3" id="videoField">
                ...
                @if(!empty($slider->video))
                    <div class="mb-2">
                        <video src="{{ asset('tema/uploads/slider/videos/' . $slider->video) }}" controls style="max-width:300px;border-radius:8px;"></video>
                    </div>
                @endif
                <input type="file" class="form-control-file" name="video" accept="video/mp4,video/webm,video/ogg">
            ...
            <div class="row mb-3">
                <div class="col-6">
                    ...
                    <input type="number" class="form-control" name="sira" value="{{ old('sira', $slider->sira) }}" style="height:46px;border-radius:8px;border:2px solid var(--admin-border);font-family:'Poppins',sans-serif;">
                </div>
                <div class="col-6 d-flex align-items-end">
                    <label style="font-size:14px;font-weight:500;cursor:pointer;">
                        <input type="checkbox" name="durum" value="1" {{ old('durum', $slider->durum) ? 'checked' : '' }} style="margin-right:6px;"> Aktif
                    </label>
                </div>
            </div>

            <button type="submit" class="btn-admin btn-admin-primary w-100" style="padding:14px;font-size:15px;border-radius:8px;">
                <i class="fas fa-save"></i> Slider'ı Güncelle
            </button>
        </form>
    </div>
</div>
