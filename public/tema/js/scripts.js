...
function scrollgoto(){
    $('.gocheck').on('click', function(event) {
        ...
        // mailto:, tel:, http://, https:// gibi external linkleri atla
        ...
        // Sadece sayfa içi anchor linklerini işle (# ile başlayan)
        if (href.indexOf('#') === 0) {
            var target = $(href);
            if( target.length ) {
...
function loadWindowEvents() {
    ...
    /*-------------------------*/
    $(window).on('scroll', function(){
      // Mobilde fixed navbar'ı devre dışı bırak
      if ($(window).width() > 768) {
        ...
        // Mobilde fixed class'ını kaldır
        $('.menu-wrap').removeClass('fixed');
      }
    });
}
