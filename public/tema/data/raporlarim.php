...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest')
{
    ...
    //Search
    $sql ="SELECT * FROM musteri_raporlar WHERE uyeid= '".$_SESSION['site_uyeid']."'";
    ...
    foreach($islem as $row)
    {
        ...
        if($row['dosya'] != '')
        {
            $dosya = '<strong><a target="_blank" href="'.tema.'/uploads/dosyalar/'.$row['dosya'].'"><i class="fa fa-file-word-o"></i> Rapor İndir</a></strong>';
        }
        else
        {
            $dosya = '';
