...
{
	$Sorgu = $db->prepare("SELECT * FROM hizmetler WHERE seo = ? AND durum = ? AND dil = ?");
...
{
	$Sorgu = $db->prepare("SELECT * FROM hizmetler WHERE durum = ? AND dil = ? ORDER BY id ASC");
...
$menubul 	= $db->query("SELECT * FROM menu WHERE menu_url = 'hizmet/".$Sonuc['seo'].".html' OR link = 'hizmet/".$Sonuc['seo'].".html' AND dil = '{$_SESSION['k_dil']}'")->fetch(PDO::FETCH_ASSOC);
$menubas 	= $db->query("SELECT * FROM menu WHERE id = '{$menubul['menu_ust']}' AND dil = '{$_SESSION['k_dil']}'")->fetch(PDO::FETCH_ASSOC);		
?>
<!-- ***** BANNER ***** -->
<div class="top-header overlay" style="background-image: url(<?php echo tema;?>/uploads/arkaplan/hizmetler/<?php echo $arkaplan['hizmetler']?>)">
    <div class="container">
        <div class="row">
            <div class="col-sm-12 col-md-12">
				<div class="wrapper">
                    <h1 class="heading"><?php echo $Sonuc['adi']?></h1>
                    <h3 class="subheading"><?=@$dil['txt304'];?></h3>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="row">
            <div class="col-sm-12">
				...
				<div class="action-content">
					<img src="<?php echo tema;?>/uploads/hizmetler/<?php echo $Sonuc['resim'];?>" alt="<?php echo $Sonuc['adi'];?>" class="img-responsive">
				</div>
				<?php }?>
